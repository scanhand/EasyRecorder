...
namespace ESR.Recorder
{
    public class ESRKeyRecorder
    {
        ...
        private IRecorderItem CurrentKeyRecorder
        {
            get
            {
                return ESRRecorder.CurrentKeyRecorder;
            }
        }

        private readonly List<VirtualKeyCode> NeedtoUpDownList = null;

        public ESRKeyRecorder(ESRRecorder recorder)
        {
        ...
        {
            if ((this.ESRRecorder.GetLastItem()?.Recorder == RecorderType.KeyUpDown) &&
                this.ESRRecorder.GetLastItem()?.Dir == Dir.Down &&
                (this.ESRRecorder.GetLastItem() as IKeyRecorderItem)?.VkCode == vkCode)
            {
                return true;
            }
        ...
        {
            List<IRecorderItem> keyDownItems = this.ESRRecorder.Items.FindAll(p => p.Recorder == RecorderType.KeyPress);
            foreach (var item in keyDownItems)
            {
